 ...
 * @author thari
 */
public class Search_products extends javax.swing.JDialog {

    Home hr;
    private static final int HEADER_HEIGHT = 27;

    public Search_products(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        this.hr = (Home) parent;
        initComponents();
        loadProducts();
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            ResultSet rs = MySQL.search("SELECT * FROM `product` INNER JOIN `brand` ON `product`.`brand_id`=`brand`.`id` INNER JOIN `category` ON `product`.`category_id`=`category`.`id` WHERE `product`.`name` LIKE '%" + jTextField1.getText() + "%' OR `product`.`barcode` LIKE '%" + jTextField1.getText() + "%'");
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
            while (rs.next()) {
                Vector v = new Vector();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                Search_products dialog = new Search_products(new javax.swing.JFrame(), true);
